...
def evaluate(model, loader, mode, cfg, update_thres=None):
    model.eval()
    ...
    with torch.no_grad():
        for img, mask, id in loader:
            ...
            if mode == 'sliding_window':
                ...
                if update_thres is not None:
                    final = update_thres.distribution_alignment(final)
                ...
                if update_thres is not None:
                    pred = model(img)
                    pred = update_thres.distribution_alignment(pred.softmax(dim=1))
